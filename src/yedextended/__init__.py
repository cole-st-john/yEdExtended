...
class Graph:
	...
	def write_graph(self, filename=None, pretty_print=False):
		"""Convert graphml xml tree into graphml file.
		Gives file default name of 'temp.graphml' if no name is given."""

		self.construct_graphml()

		if not filename:
			filename = "temp.graphml"
	...
	def from_existing_graph(self, file_path):  # TODO:
		"""Manage ingestion of existing graph file"""

		if file_path is None:
			raise ValueError(
				"A graphml filepath must be provided as basis for the graph."
			)

		try:
			with open(file_path, "r") as graph_file:
				graphml_string = graph_file.read()
...
def is_yed_findable():
	...
	if not yed_found_bool:
		tk.Message(text="Could not find yEd application.")
...
def maximize_graph(window) -> None:
	window.activate()
	window.maximize()
	window.show()
...
def get_yed_graph_window_id(file):
	"""Retrieve id of yed window containing file"""
	file = os.path.realpath(file)
	file_dir = os.path.dirname(file)
	file_name = os.path.basename(file)
	file_name_wo_ext = os.path.splitext(file_name)[0] or ""
	potential_windows = [title.lower for title in gw.getAllTitles()]
	search_name = file_name_wo_ext + "- yEd"
	window = gw.getWindowsWithTitle(search_name)[
		0
	]  # FIXME: this would give a false positive for files of same name but diff path.

	return window or None


def is_yed_graph_open(file) -> bool:
	"""Check whether yEd is currently open - windows, linux, os, etc."""
	window = get_yed_graph_window_id(file)
	return window is not None


def open_yed_file(file):
